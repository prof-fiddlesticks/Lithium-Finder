...
print("1. Authenticated successfully. Building the 100-site 3D Topo-Dataset...")

# 2. THE EXPANDED COORDINATES (100 Sites)
locations = [
...
# 3. LOAD SENTINEL-2 AND SRTM TOPOGRAPHY
print("2. Requesting Sentinel-2 Imagery and SRTM Topography...")
...
# 4. DEFINE THE PLAN C MARKERS
def get_plan_c_markers(image):
    b = {
        'B2': image.select('B2'), 'B3': image.select('B3'),
        'B4': image.select('B4'), 'B8': image.select('B8'),
        'B8A': image.select('B8A'), 'B11': image.select('B11'), 'B12': image.select('B12')
    }
    ...
    # Add everything together: Ratios + Raw SWIR + Topography
    return image.addBands([
        clay_cmr, hydrox, silica, borate, si1, bmi, albedo, iron_ox, ferrous, ndsi,
        b['B11'], b['B12'], srtm.rename('Elevation'), slope.rename('Slope')
    ])

full_stack = get_plan_c_markers(s2)

# 5. EXTRACT THE DATA
print("3. Extracting Geological + Topographical data...")
features_to_extract = [
...
training_set = full_stack.select(features_to_extract).sampleRegions(
    collection=pts, properties=['Site', 'Label'], scale=20, tileScale=4
)

# 6. EXPORT TO CSV
output_file = os.path.join(os.getcwd(), 'master_lithium_v3_context.csv')
geemap.ee_to_csv(training_set, filename=output_file)
print(f"✅ SUCCESS! Plan C Dataset saved to: {output_file}")
